...
请识别图片中每个独立的物品，返回它们的边界框坐标和建议旋转角度。
严格按照以下 JSON 数组格式输出，不要输出任何其他内容：

[
  {"bbox": [x1, y1, x2, y2], "label": "物品1", "suggested_rotation": 0},
  {"bbox": [x1, y1, x2, y2], "label": "物品2", "suggested_rotation": 90}
]
...
4) suggested_rotation 是让该物品的包装/标签文字旋转到正常可读位置所需的顺时针旋转角度，只能取 0、90、180、270 中的一个：
   - 0 表示文字已经正向，无需旋转
   - 90 表示需要顺时针旋转 90 度文字才正向
   - 180 表示需要旋转 180 度（当前倒置）
   - 270 表示需要顺时针旋转 270 度
   判断依据：观察物品包装上的文字、标签、品牌名等，以文字的正常阅读方向为准。
   如果物品上没有文字或无法判断，返回 0。
