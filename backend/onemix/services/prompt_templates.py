...
OBJECT_DETECT_BBOX_RETRY_PROMPT = """\
请仔细观察这张图片，图中是否有多个独立物品？

注意：
- 即使物品叠放、部分遮挡，也是多个独立物品
- 即使物品相邻、紧贴，也是多个独立物品
- 请分别返回每个物品的 bbox
...
要求：
1) bbox 为 [左上角x, 左上角y, 右下角x, 右下角y]，基于原图像素坐标。
2) 被遮挡的物品只标注其可见部分的 bbox，不要估算被遮挡部分。
3) 各物品的 bbox 不要重叠。
4) 如果图中确实只有 1 个物品，也按数组格式返回单个元素。
5) 只输出 JSON 数组，不要 Markdown 代码块，不要解释文字。
"""
